...
void SystemTrayIcon::Show()
{
	...
	NotifyIconData.dwState &= (0xFFFFFFFF ^ NIS_HIDDEN);
	Update();
}
...
void SystemTrayIcon::Focus()
{
	if (!bInitialized) return;
	bLastResult = Shell_NotifyIconW(NIM_SETFOCUS, &NotifyIconData);
}

void SystemTrayIcon::Delete()
{
	if (!bInitialized) return;
	bLastResult = Shell_NotifyIconW(NIM_DELETE, &NotifyIconData);
	bInitialized = false;
}

void SystemTrayIcon::Update(HICON					hIcon,
